...
GITHUB_REPONAME    = "darioplatania/mysite"
...
namespace :draft do
  ...
  task :new do
    ...
    FileUtils.touch("source/_drafts/#{@slug}.markdown")
    open("source/_drafts/#{@slug}.markdown", 'a' ) do |file|
      file.puts "---"
  ...
  task :ready do
    ...
    Dir.foreach("source/_drafts") do |fname|
    ...
    FileUtils.mv("source/_drafts/#{@post_name}", "source/_posts/#{@post_name}")
    FileUtils.mv("source/_posts/#{@post_name}", "source/_posts/#{@post_date}-#{@post_name}")
